...
TEST_CASE("Secure computation function-independent") {
  SECTION("Hand-crafted circuit") {
    ...
    std::size_t threshold = 8; // has to be even
    ...
    // multi-threading for circuit-independent preprocessing
    std::vector<std::thread> t_fi_prep(n_parties);
    PARTY {
      t_fi_prep[i] = std::thread(&tp::Circuit::FIPrepFromDummyOle, &circuits[i]);
    }
