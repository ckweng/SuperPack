...
static inline void ValidateVecSize(std::uint32_t s) {
  if (s > MAX_VEC_READ_SIZE)
    throw std::logic_error("received vector exceeds size limit");
}

static inline void ValidateMatSize(std::uint32_t r, std::uint32_t c) {
  if (r * c > MAX_MAT_READ_SIZE)
