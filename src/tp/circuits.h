...
namespace tp {
  ...
  public:
    ...
    // Append input gates
    std::shared_ptr<InputBatch> Input(std::size_t owner_id);

    // Consolidates the batches for the inputs
    void CloseInputs() { for (auto input_layer : mInputLayers) input_layer.Close(); }

    // Append output gates
    std::shared_ptr<OutputBatch> Output(std::size_t owner_id, std::vector<std::shared_ptr<Gate>> output);
    ...
    std::shared_ptr<InputGate> GetInputGate(
        std::size_t owner_id,
        std::size_t batch_idx,
        std::size_t gate_idx) {
      return mFlatInputBatches[owner_id][batch_idx]->GetInputGate(gate_idx);
    }
    std::shared_ptr<InputBatch> GetInputBatch(
        std::size_t owner_id,
        std::size_t batch_idx) {
      return mFlatInputBatches[owner_id][batch_idx];
    }
    std::shared_ptr<InputGate> GetInputGate(std::size_t idx) { return mInputGates[idx]; }

    std::shared_ptr<OutputGate> GetOutputGate(
        std::size_t owner_id,
        std::size_t batch_idx,
        std::size_t gate_idx) {
      return mFlatOutputBatches[owner_id][batch_idx]->GetOutputGate(gate_idx);
    }
    std::shared_ptr<OutputBatch> GetOutputBatch(
        std::size_t owner_id,
        std::size_t batch_idx) {
      return mFlatOutputBatches[owner_id][batch_idx];
    }
    ...
    // Dummy circuit-independent preprocessing
    void _DummyPrepFI(FF lambda);
    ...
    // Semi-Dummy FI Prep, assume dummy ole and vole
    void FIPrepFromDummyOle() {
      mCorrelator.PrepFromDummyOle();
    }
    ...
    void SetInputs(std::vector<std::vector<FF>> inputs);

    // Input protocol
    void InputPartiesRevealShares();
    void InputClientReconstructAndDistributes();
    void InputPartiesReceive();
    void InputClientSendMuToP1();
    void InputP1ReceiveMu();
    void InputPartiesAuthenticateMu();

    void RunInput();

    // Multiplications in the i-th layer
    void MultP1Distribute(std::size_t layer);
    void MultPartiesReceive(std::size_t layer);
    void MultPartiesMultiplyAndSend(std::size_t layer);
    void MultP1ReceivesAndReconstruct(std::size_t layer);
    void MultPartiesCheckAndComputeAuthMu(std::size_t layer);

    void RunMult(std::size_t layer);

    // Output layers
    void OutputPartiesRevealShares();
    void OutputP1ReconstructAndDistributes();
    void OutputPartiesReceiveShares();
    void OutputPartiesVerify();
    void OutputPartiesCheckTheta();
    void OutputPartiesRevealOutputVal();
    void OutputClientReconstructSecret();
    void RunOutput();
  ...
  private:
    ...
    std::vector<std::vector<std::shared_ptr<InputBatch>>> mFlatInputBatches; // Outer idx: client
    std::vector<std::vector<std::shared_ptr<OutputBatch>>> mFlatOutputBatches; // Outer idx: client
