...
namespace tp {
  Circuit Circuit::FromConfig(CircuitConfig config) {
    ...
    std::size_t ctr(0);
    for (std::size_t i = 0; i < config.n_parties; i++) {
      for (std::size_t j = 0; j < config.out_gates[i]/circuit.mBatchSize; j++) {
        std::vector<std::shared_ptr<Gate>> vec_out;
        for(std::size_t k = 0; k < circuit.mBatchSize; ++k) {
          auto x = circuit.GetMultGate(config.depth - 1, ctr % config.width);
          vec_out.push_back(x);
          ctr++;
        }
	circuit.Output(i,vec_out);
      }
    }
