...
namespace tp {
  ...
  public:
    ...
    static void SetPackedShrDelta(FF packed_shr_delta) {
      mPackedShrDelta.Emplace(packed_shr_delta);
    }

  protected:
    // Bool that indicates whether P1 learned Mu already
    bool mLearned = false;
    bool mAuthLearned = false;
    ...
    // The share of delta common for all batches
    static Vec mPackedShrDelta;
  ...
  public:
    ...
    FF GetDeltaMu() {
      if ( !mAuthLearned ) {
        mDeltaMu = mLeft->GetDeltaMu() + mRight->GetDeltaMu();
        mAuthLearned = true;
      }
      return mDeltaMu;
    }
