...
namespace tp {
  ...
  public:
    ...
    void SetValue(FF val) { mValue = val; };
  ...
  public:
    ...
    void SetPreprocessing(FF packed_shr_lambda,
      ...
      mAddShrLambdaDelta = Vec(add_shr_delta_lambda.begin(),
    ...
    // Online protocol
    // Parties send to P1 shares of \lambda_\alpha - a
    void PartiesRevealShares();
    // P1 reconstructs and distributes the shares
    void P1ReconstructAndDistributes();
    // Parties receive packed shares
    void PartiesReceiveShares();
    // Parties verify the output
    void PartiesVerify(std::vector<FF> &theta_list);
    void PartiesCheckTheta();
    // Parties reveal the output values
    void PartiesRevealOutputVal();
    // Client reconstruct the output values
    void ClientReconstructSecret();

    FF GetPackedShrLambda() { return mPackedShrLambda; }
    FF GetPackedShrLambdaDelta() {return mPackedShrLambda; }

    FF GetPackedShrDelta(std::size_t i) {
      return OutputGate::GetPackedShrDelta(i);
    }
  ...
  public:
    ...
    void Append(std::shared_ptr<OutputBatch> output_batch) {
      mBatches.emplace_back(output_batch);
    }
    ...
    /*void _DummyPrep(FF lambda, FF delta) {
      for (auto batch : mBatches) batch->_DummyPrep(lambda, delta);
    }
    void _DummyPrep() {
